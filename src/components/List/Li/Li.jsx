...
const Li = props => {
  ...
  const styleSelect = {
      backgroundColor: isSelect && '#CCF0E6',
      borderRadius: isSelect && '10px'
  }
    ...
    <ol>
      <li value={id} style = {{...styleSelect, ...styleDelete}} onClick={()=>setSelect(selected=>!selected)}>
        ...
        <div className={styles.li}>
          ...
          />
          <button className={`${styles.btn} ${styles.btnLightGreen}`}
          onClick = {()=>setEdit(false)}>Сохранить</button>
          <button className={`${styles.btn} ${styles.btnGreen}`} onClick = {()=>setEdit(false)}>Отмена</button>
        </div> :
        <div className={styles.li}>
          ...
          <button className={`${styles.btn} ${styles.btnOrange}`} onClick = {()=>setDelete(true)}>Удалить</button>
        </div>
        }
      </li>
    </ol>
